		  ...
		  int *total, bool speed)
{
  ...
  *total = 0;
  switch (code)
	...
	if (GET_CODE(a) == PLUS)
	  {
	    ...
	    if (GET_CODE(b) == SYMBOL_REF || GET_CODE(b) == LABEL_REF)
	      {
		*total = GET_MODE_UNIT_SIZE(mode) > 2 ? 6 : 4;
		return true;
	      }
	    *total = 9;
	    return true;
	...
	*total = GET_MODE_UNIT_SIZE(mode) > 2 ? 5 : 3;
      return true;
    case CONST_DOUBLE:
      *total = GET_MODE_UNIT_SIZE(mode) > 4 ? 10 : 5;
      return true;
    case POST_INC:
      *total = 0;
      return true;
    ...
    case SIGN_EXTEND:
      *total = 4;
      return true;
    case UDIV:
      *total = GET_MODE_UNIT_SIZE(mode) > 2 ? 79 : 44;
      return true;
    case MOD:
    case DIV:
      *total = GET_MODE_UNIT_SIZE(mode) > 2 ? 91 : 57;
      return true;
    case MEM:
      {
	rtx a = XEXP(x, 0);
	...
	*total = GET_MODE_UNIT_SIZE(mode) > 2 ? 44 : 28;
	return true;
      }
      break;
    case COMPARE:
