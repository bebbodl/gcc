		...
		int *total, bool speed )
{
  ...
  switch (code)
	    ...
	    && m68k_68000_10_costs (b, mode, code, 1, &total2, speed))
	  {
	    *total += total2;
	    if (MEM_P(a) && GET_CODE(b) == CONST_INT && INTVAL(b) == 0)
	      {
		// penalty for clr
		*total += GET_MODE_UNIT_SIZE(mode) > 2 ? 8 : 4;
	...
	*total = 0;
	...
	if (speed)
	  {
	    int f = GET_MODE_UNIT_SIZE(mode) > 2 ? 150 : 50;
	    if (GET_CODE(b) == CONST_INT && GET_MODE_UNIT_SIZE(mode) == 2)
	      {
		unsigned i = INTVAL(b);
		int bits = 0, l = 0;
		if (i > 0)
	  ...
	  *total = GET_MODE_UNIT_SIZE(mode) > 2 ? 48 : 16;
	...
	if (m68k_68000_10_costs (a, mode, code, 0, total, speed))
	  {
	    if (speed)
	      *total += GET_MODE_UNIT_SIZE(mode) > 2 ? 410 : 136;
	    else
	      *total += GET_MODE_UNIT_SIZE(mode) > 2 ? 108 : 36;
	    return true;
	  }
	break;
      }
	...
	if (REG_P(a))
	  {
	    if (GET_CODE(b) == CONST_INT)
	      {
		*total = (GET_MODE_UNIT_SIZE(mode) > 2 ? 8 : 6);
		*total += 2 * INTVAL(b);
		return true;
	      }
	    *total = (GET_MODE_UNIT_SIZE(mode) > 2 ? 8 : 6) ;
	...
	if (kind == 0)
	  ...
	  *total = GET_MODE_UNIT_SIZE(mode) > 2 ? 10 : 8;
	else
	  *total = GET_MODE_UNIT_SIZE(mode) > 2 ? 12 : 8;
	return true;
      }
    case NEG:
    case NOT:
	...
	if (GET_CODE(b) == CONST_INT)
	  {
	    ...
	    if (REG_P(a) && (USE_MOVQ(i) || (REGNO(a) >= 8 && REGNO(a) < 16 && (unsigned)(INTVAL(b) + 32768) < 65535)))
	      ...
	      *total = GET_MODE_UNIT_SIZE(mode) > 2 ? 16 : 8;
	    return true;
	  }
	...
	*total += total2 + GET_MODE_UNIT_SIZE(mode) > 2 ? 6 : 4;
	return true;
      }
    case COMPARE:
    case AND:
	...
	if (GET_CODE(b) == CONST_INT)
	  {
	    if (REG_P(a))
	      {
		*total = GET_MODE_UNIT_SIZE(mode) > 2 ? (code == AND ? 14 : 16) : 8;
		return true;
	      }
	    if (m68k_68000_10_costs (a, mode, code, 0, total, speed))
	      {
		*total += GET_MODE_UNIT_SIZE(mode) > 2 ? 20 : 12;
		return true;
	      }
	  }
	else
	if (REG_P(a))
	  {
	    if (m68k_68000_10_costs (b, mode, code, 1, total, speed))
	      {
		*total += GET_MODE_UNIT_SIZE(mode) > 2 ? 4 : (code == XOR ? 8 : 6);
		return true;
	      }
	  }
	else
	...
	if (m68k_68000_10_costs (a, mode, code, 0, total, speed)
	    && m68k_68000_10_costs (b, mode, code, 1, &total2, speed))
	  {
	    *total += total2 + GET_MODE_UNIT_SIZE(mode) > 2 ? 4 : (code == XOR ? 8 : 6);
	    return true;
	  }
	break;
      }
